...
export default function Home() {
        ...
        children: [
          {
            ...
            children: [
              {
                label:
                  "Main Elements - Main Dashboard, Journey  & Calendar Dashboard (+Progress?)",
                value:
                  "Main Elements - Main Dashboard, Journey  & Calendar Dashboard (+Progress?)",
                isSelected: false,
                isDisabled: false,
              ...
              {
                label:
                  "Secondary Elements - Foundation Board, Vision Board, Playground, Progress",
                value:
                  "Secondary Elements - Foundation Board, Vision Board, Playground, Progress",
                isSelected: false,
                isDisabled: false,
  ...
  const handleCreateSprint = () => {
    ...
    const newProject = {
      position:
        projects.length > 0 ? projects[projects.length - 1].position + 1 : 1,
  ...
  useEffect(() => {
    fetch("/api/tasks/task")
      ...
      .then((data) => {
        console.log(data);
        let all_project = data?.currentMonthTasks?.map((project,i) => {
          // format my date
          let startDateFormated = moment(project?.created_at).format(
            "MM/DD/YYYY"
          );
          let endDateFormated = moment(project?.end_date || new Date()).format(
            "MM/DD/YYYY"
          );
          let new_project = {
            position:i,
            ...
            level: project?.label,
            startDate: startDateFormated || formatDate(new Date()),
            endDate: endDateFormated || formatDate(new Date()),
            color: project?.color ?? "#A05700",
  ...
  const updateProjectDetails = (newProject) => {
    if (newProject.isCompleted) {
      completeProjectAndUpdatePostions1({
        ...newProject,
  ...
  const completeProjectAndUpdatePostions1 = async (project) => {
    ...
    const completedPostion =
      completedProjects.length > 0
        ? completedProjects[completedProjects.length - 1].position + 1
        : 1;
    console.log(
      "completedPostion",
      completedPostion,
  ...
  return (
    <main className={styles.main}>
      <div
        ref={offCanvasRef}
        className={`offcanvas offcanvas-end w-50 ${styles.offCavas}`}
        tabIndex="-1"
        id="offcanvasRight"
        ...
        <div className={`offcanvas-body ${styles.offCavasBody}`}>
          <div className={`${styles.offCavasBodyContent}`}>
            ...
            <input
              ...
              type="text"
              placeholder="🎯 Sprint #1 - Create Basic Layout for Kaizen Flow"
            />
            ...
            <div className={`${styles.levelSelectWrapper}`}>
              <label
                className={`${styles.levelInputTitle}`}
                htmlhtmlFor="level"
              >
                Which Level
              </label>
              <select
                value={selectedProject ? projectLevel : null}
              ...
              <div className={`${styles.dateWrapper}`}>
                ...
                <div className={`${styles.datePickerWrapper}`}>
                  ...
                  <input
                    value={
                      projectEndDate
                        ? new Date(projectEndDate)?.toISOString()?.substr(0, 10)
                        : ""
                    }
                    onChange={(e) => setProjecEndDate(e.target.value)}
                    className={`${styles.datePicker}`}
                    type="date"
                    id="date"
                    name="date"
            ...
            <textarea
              ...
              className={`my-4 ${styles.projectDescriptionTextArea}`}
              rows={2}
              placeholder=""
            ></textarea>

            <h2 className={`${styles.todoHeading}`}>To Get Done This Week:</h2>
              ...
              <NestedChecklist
                data={
                  selectedProject?.checklist
                    ? selectedProject.checklist.children
                    : checklistData.children
                }
      ...
      <div className={`${styles.sprintBtnNav}`}>
        <button
          className={`${styles.btnSecondary} ${styles.dotted}`}
      ...
      <div className={styles.content}>
            ...
            return (
              ...
              >
                              ...
                              {project && !project.isCompleted && (
                                ...
                                >
                                  <div
                                    ...
                                    onClick={() =>
                                      completeProjectAndUpdatePostions(project)
                                    }
                                    data-bs-toggle="offcanvas"
                                    data-bs-target="#offcanvasRight"
                                    style={{
                                      backgroundColor: project.color,
                                      width: `calc(47px * ${project.duration})`,
                                    }}
                                  >
                ...
                <div className={styles.sprintsCompleted}>
                  <div className={`${styles.column} ${styles.completed}`}></div>
                  {[...Array(month.days).keys()].map((day, daysIndex) => {
                    let foundProjectsIndex = 0;
                    const monthNumber = monthIndex + 1;
                    const dayNumber = daysIndex + 1;
                          ...
                          return (
                            <>
                              {project && project.isCompleted && (
                                ...
                                >
                                  <div
                                    className={`${styles.sprint} ${styles.completedSprint}`}
                                    style={{
                                      width: `calc(47px * ${project.duration})`,
                                    }}
                                  >
        ...
        <div className={`${styles.levelsCompletedContainer}`}>
          {[...Array(10).keys()].map((level, index) => {
            ...
            if (!foundProjects) {
              const foundCompletedProjects = completedProjects.find(
                (project) => {
                  return project.level == level;
                }
              );
            ...
            return (
              <div
                key={index}
                className={`${styles.levelCompletedWrapper} ${
                  isLevelCompleted ? styles.levelDisplay : "d-none"
                }`}
              >
                <div
                  className={`${styles.levelCompletedTitle}`}
                >{`Level ${level} Completed`}</div>
                <Image
                  src="/images/checkGreen.svg"
                  alt="level complete icon"
                  width={24}
                  height={24}
                />
              </div>
